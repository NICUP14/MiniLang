...
fun replace(pattern: c_str, from: c_str, repl: c_str): str
    let mlen = 0
    ...
    let rest = empty_str
    while idx >= 0
        sub = empty_str if idx - 1 < 0 else curr.substr(0, idx - 1)
        rest = curr.substr(idx + len(sep), 0 - 1)
        s = (s.concat(sub).
               concat(sep))
...
fun unwrap(arg: match_info): int64
    if arg.match_success == false
        panic("Attempt to grab index fom unsucesfull match")
    end
